...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `ping` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `amar-ysabie` with the `ORG` of your provider.
   - replace all instances of the `github.com/amar-ysabie/pulumi-ping` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-ping
  ...
  ignore: []
  ldflags:
  - -X github.com/amar-ysabie/pulumi-ping/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-ping/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-ping_darwin_amd64_v1
│   └── pulumi-resource-ping
├── pulumi-ping_darwin_arm64
│   └── pulumi-resource-ping
├── pulumi-ping_linux_amd64_v1
│   └── pulumi-resource-ping
├── pulumi-ping_linux_arm64
│   └── pulumi-resource-ping
├── pulumi-ping_windows_amd64_v1
│   └── pulumi-resource-ping.exe
└── pulumi-ping_windows_arm64
    └── pulumi-resource-ping.exe
```
